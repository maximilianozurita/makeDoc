...
Generates or updates professional technical documentation for GitHub projects.
Writes files directly to the filesystem of the current project.

Operation modes:
- `/makeDoc` → generate documentation from scratch
- `/makeDoc --update` → update existing documentation without overwriting content that is still valid

---

## When to use this skill

When `/makeDoc` or `/makeDoc --update` is invoked, or when the user asks to generate
or update technical documentation for a GitHub project.
...
Before anything else, check whether the command was invoked with `--update`.
...
- If a section has information you cannot verify in the current code, preserve it
  without marking it — just leave it as it was.
- Do not delete existing files; if a file no longer applies (e.g., `api.md` and the API was removed),
  warn the user but do not delete it automatically
...
- `docs/usage.md` → if the project has a user interface, non-trivial usage flows, or CLI commands that an end user needs to understand
...
## Quick installation
Summary in 2-3 steps. Link to [docs/installation.md](docs/installation.md)

## Architecture (summary)
Short paragraph. Link to [docs/architecture.md](docs/architecture.md)

## Project structure
Summarized tree. Link to [docs/structure.md](docs/structure.md)
...
Aimed at the end user or the developer who will use the project, not the one who will modify it.

- How the project is used in practice (app, service, script, CLI, library, etc.)
...
- **If something is not in the code, omit it**: do not leave banners like
  `⚠️ Not verified` or visible warnings in generated files.
  If a section lacks sufficient evidence in the code, simply do not write it.
  Report those omissions only in the final summary to the user, not inside the files.
...
- **Avoid obvious explanations** (do not explain what a `for loop` is)
...
## Special focus on architecture and structure

- Highlight separation of concerns
- Explain how modules interact with each other
- Point out good practices where they exist
- If the architecture is weak or inconsistent, state it with technical judgment
...
Report to the user:
- What files were generated or updated
- What sections were preserved unchanged (in `--update` mode)
- What sections were omitted due to lack of evidence in the code, and why
- If there are files that could be deleted because they no longer apply (in `--update` mode)
- If anything needs to be completed manually
